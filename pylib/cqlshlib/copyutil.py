...
class ImportErrorHandler(object):
    ...
    @property
    def err_file(self) -> int:
        return os.open(self.err_filename, os.O_CREAT | os.O_APPEND | os.O_WRONLY, 0o644)
    ...
    def add_failed_rows(self, rows):
        ...
        try:
            with os.fdopen(self.err_file, "a", buffering=1) as f:
                writer = csv.writer(f, **self.options.dialect)
                for row in rows:
                    writer.writerow(row)
        except Exception as exc:
            printdebugmsg("Warning: failed to write failed rows to error file %s" % self.err_filename)
            if self.shell.debug:
                traceback.print_exc()
            raise
